/**
 * Star Wars Edge Studio Game System
...
/**
 * Generate a Star Wars Edge Studio-standardized document ID given a provided string title.
...
async function syncTalents(force = false) {
    console.groupCollapsed("Star Wars Edge Studio | Talent Data Synchronization")
    ...
    for (const actor of game.actors) {
        ...
        if (force || foundry.utils.isNewerVersion(game.system.version, actor._stats.systemVersion)) {
            try {
                ...
                console.log(`Star Wars Edge Studio | Synchronized talents for Actor "${actor.name}"`);
                synced++;
            } catch (err) {
                console.warn(`Star Wars Edge Studio | Talent synchronization failed for Actor "${actor.name}": ${err.message}`);
            }
    ...
    console.log(`Star Wars Edge Studio | Complete talent synchronization for ${synced} Actors`);
    console.groupEnd();
}
